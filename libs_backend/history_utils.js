(function(){
  ...
  out$.ensure_history_utils_data_cached = ensure_history_utils_data_cached = async function(){
    var date_now, pages_list_compressed, pages_list, baseline_session_time_on_domains, baseline_time_on_domains, url_to_visits;
    date_now = Date.now();
    if (localStorage.cached_pages_list !== 'true') {
      pages_list_compressed = (await getvar_history('pages_list_compressed'));
      if (pages_list_compressed == null) {
        pages_list = (await new Promise(function(it){
          return chrome.history.search({
            text: '',
            startTime: 0,
